...
def predict_target_price(target_type):
    ...
    df3 = pyupbit.get_ohlcv(ticker, interval="minute360", count=81, to=df2.index[0])
    ...
    X_train = []
    y_train = []
    for i in range(480, len(X)):
        X_train.append(X[i - 480:i, :])
