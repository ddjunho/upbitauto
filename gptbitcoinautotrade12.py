...
def predict_target_price(target_type):
    ...
    df3 = pyupbit.get_ohlcv(ticker, interval="minute360", count=50, to=df2.index[0])
    ...
    X_train = []
    y_train = []
    for i in range(449, len(X)):
        X_train.append(X[i - 449:i, :])
        y_train.append(y[i, 0])
    X_train = np.array(X_train)
    y_train = np.array(y_train)
    # Tensorflow 모델 구성
    model = tf.keras.models.Sequential([
        tf.keras.layers.LSTM(128, input_shape=(449, 5)),
    ...
    # 새로운 데이터에 대한 예측
    last_data = DF[['open', 'high', 'low', 'close', 'volume']].iloc[-449:].values
