...
def predict_target_price(target_type):
    ...
    df3 = pyupbit.get_ohlcv(ticker, interval="minute360", count=100, to=df2.index[0])
    ...
    for i in range(499, len(X)):
        X_train.append(X[i - 499:i, :])
        y_train.append(y[i, 0])
    X_train = np.array(X_train)
    y_train = np.array(y_train)
    # Tensorflow 모델 구성
    model = tf.keras.models.Sequential([
        tf.keras.layers.LSTM(128, input_shape=(499, 5)),
...
while True:
    try:
        ...
        if now.hour in [3, 9, 15, 21] and now.minute == 0:
            ...
            PriceEase = round((sell_price - target_price) * 0, 1)
            bull_market = is_bull_market(COIN)
        if now.hour < 3:
